 ...
 * </pre>
 */
public class Tensor3d extends DfData {
    private double[][][] data;
    private int dims;
     ...
     */
    public Tensor3d(int dim1, int dim2, int dim3, double filler) {
        dims = 3;
     ...
     * @param dim3 The z size or depth of the array.
     */
    public Tensor3d(int dim1, int dim2, int dim3) {
        dims = 3;
     ...
     * @return
     */
    public static Tensor3d zeros(int dim1, int dim2, int dim3) {
        return new Tensor3d(dim1, dim2, dim3, 0);
    }
     ...
     * @return
     */
    public static Tensor3d ones(int dim1, int dim2, int dim3) {
        return new Tensor3d(dim1, dim2, dim3, 1);
    }
     ...
     * </pre>
     */
    public Tensor3d slice(Range x1Range, Range x2Range, Range x3Range) {
     ...
     * </pre>
     */
    public void append(Tensor3d toAppend, int dim) {
        if (dim < 0 || dim > dims) {
     ...
     * </pre>
     */
    public void append(Tensor2d toAppend) {
     ...
     * </pre>
     */
    public static Tensor3d matmul(Tensor3d mul, Tensor3d toMul) {
        checkDims(mul, toMul);
        return new Tensor3d(matmul(mul.getData(), toMul.getData()));
    }
    ...
    public static double[][][] matmul(double[][][] mul, double[][][] toMul) {
        ...
        for (int x3 = 0; x3 < mul.length; x3++) {
            result[x3] =  Tensor2d.matmul(mul[x3], toMul[x3], MUL_TILE_SIZE);
        }
     ...
     * </pre>
     */
    public Tensor2d squeeze(int dim) {
        int[] shape = shape();
        if (shape[dim] > 1) {
            throw new DataException("Dim 3 must be of size 1 or 0 but was " + shape[dim]);
        }
